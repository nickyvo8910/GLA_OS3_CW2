...
// Frances Ramirez
// 2168154R
// Operating System 2 Assessed Exc | University of Glasgow
////////////////////////////////////////////////////////////////////////////////
...
// START of assignment code section 
    ...
    //mutex created to protext critical sections
...
int run_driver(CLObject* ocl,unsigned int buffer_size,
                int* input_buffer_1, int* input_buffer_2, int* output_buffer) {
    
    long long unsigned int tid = ocl->thread_num;

    // printf("Vrun_driver thread: %llu\n",tid);
...
// START of assignment code section 
    ...
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error writing input1 data into device memory: %d!\n",err);
        exit(EXIT_FAILURE);
     }
    err = clEnqueueWriteBuffer(ocl->command_queue, input2, CL_TRUE, 0,
                                buffer_size*sizeof(int), input_buffer_2, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error writing input2 data into device memory: %d!\n",err);
        exit(EXIT_FAILURE);
     }
    ...
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error setting argument at index of input1 to kernel: %d!\n",err);
        exit(EXIT_FAILURE);
     }

    err = clSetKernelArg(ocl->kernel, 1, sizeof(cl_mem), &(input2));
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error setting argument at index of input2 to kernel: %d!\n",err);
        exit(EXIT_FAILURE);
     }

    err = clSetKernelArg(ocl->kernel, 2, sizeof(cl_mem), &(output));
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error setting argument at index of output to kernel: %d!\n",err);
        exit(EXIT_FAILURE);
     }
     
    err = clSetKernelArg(ocl->kernel, 3, sizeof(cl_mem), &(status_buf));
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error setting argument at index of status_buf to kernel: %d!\n",err);
        exit(EXIT_FAILURE);
     }

    err = clSetKernelArg(ocl->kernel, 4, sizeof(unsigned int), &(buffer_size));
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error setting argument at index of buffer_size to kernel: %d!\n",err);
        exit(EXIT_FAILURE);
     }
    ...
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error blocking using method clFinish(): %d!\n",err);
        exit(EXIT_FAILURE);
     }
    ...
    err = clEnqueueReadBuffer(ocl->command_queue, status_buf, CL_TRUE, 0, 
                            sizeof(int), &temp_status, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error reading status into a temporary variable: %d!\n",err);
        exit(EXIT_FAILURE);
     }
    if (temp_status == 0) {
    ...
    // Shutdown and cleanup // [YOUR CODE HERE]
    ...
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error releasing status_buf buffer: %d!\n",err);
        exit(EXIT_FAILURE);
     }
